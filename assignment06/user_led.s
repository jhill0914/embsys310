...
control_user_led1
    ...
    LDR.N R2, =GPIOA_BASE + GPIOA_ODR
    STR R3, [R2]
    
    CMP R0, #0          // compare values of R2 (*c1) and R3 (*c2)
    ITE EQ              // IF-THEN block
    BICEQ R3, R3, #GPIOA_BIT_5     // if R2 and R3 are not equal, set R0 to 1
    ORRNE R3, R3, #GPIOA_BIT_5  // set bit 5 using bitwise OR
